...
class PosOrder(models.Model):
    ...
    def _post_reconciliation_message(self, invoice, reconciliation_details):
        ...
        message_lines = ["<strong>Détails de la réconciliation des commandes POS :</strong><br/><br/>"]
        ...
        for detail in reconciliation_details:
            ...
            message_lines.append(
                "📋 <strong>{order_name}</strong> - {amount} {currency}<br/>"
                "├─ Extourne : {reversal_link}<br/>"
                "└─ Session : {session_link}<br/><br/>".format(
                    order_name=order.name,
                    amount=order.amount_total,
                    currency=order.currency_id.symbol,
                    reversal_link=reversal_move._get_html_link(),
                    session_link=session_move._get_html_link(),
                )
            )

        message_body = Markup(''.join(message_lines))
        invoice.message_post(body=message_body)
    ...
    def prepare_invoice_lines_settle(self, orders):
        ...
        for order in orders:
            ...
            # Use Odoo 18 method to prepare invoice lines
            line_values_list = order._prepare_tax_base_line_values()
            for line_values in line_values_list:
                line = line_values['record']

                # Skip combo products (handled differently in Odoo 18)
                if line.product_id.type == 'combo':
                    continue

                # Get invoice line values using Odoo 18 method
                invoice_line_vals = order._get_invoice_lines_values(line_values, line)
                invoice_lines.append((0, None, invoice_line_vals))

                # Check if we need to add discount note (Odoo 18 uses percentage check)
